...
public class AdminOnlyFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
     ...
     // We need HTTP request objects
        ...
        if (!CheckDatabase.isInBlackList((String)hrequest.getSession().getAttribute("username")) && hrequest.getSession().getAttribute("openid")!=null && hrequest.getSession().getAttribute("login") != null && hrequest.getSession().getAttribute("login").equals("true") && CheckDatabase.isAdmin((String)hrequest.getSession().getAttribute("username"))){
        	chain.doFilter(request, response);
        }else if (CheckDatabase.isInBlackList((String)hrequest.getSession().getAttribute("username"))){
        	Out.println("Out of service");
        }
        else{
        	System.out.println("Filter:"+hrequest.getRequestURL());
        	request.getRequestDispatcher("/auth/openid?op=Google").forward(request, response);
        }
        /*else{
