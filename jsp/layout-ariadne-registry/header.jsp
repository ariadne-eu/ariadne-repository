...
<%@page import="org.ariadne_eu.utils.registry.auth.CheckDatabase"%>
<%@page import="org.ariadne.config.PropertiesManager"%>
<%@page import="org.ariadne_eu.utils.config.RepositoryConstants"%><div id="wrapper">
    <div id="header">
        <div id="mambo">
...
<%
	...
	boolean isAdmin =  CheckDatabase.isAdmin((String)request.getSession().getAttribute("username"));
    ...
    <td class="menubackgr">
		...
		[			
			...
			[null,'Modify',null,null,'Insert-Modify',
				['<img src="<%=request.getContextPath()%>/includes/js/ThemeOffice/content.png" />','Insert-Modify Metadata','<%=request.getContextPath()%>/admin-ariadne-registry/updateMetadata.jsp',null,'Insert-Modify Metadata'],
			],			
<%			
...
%>			
		[null, 'Utils', null, null, 'Utils',
				...
				['<img src="<%=request.getContextPath()%>/includes/js/ThemeOffice/chart.png" />','Small vis','<%=request.getContextPath()%>/rss/VisualizationCatalogs.jsp',null,''],
		],  
			_cmSplit,		
...
<%
        if (!isLoggedIn) {
%>
            <a href="<%out.println(request.getContextPath()+PropertiesManager.getInstance().getProperty(RepositoryConstants.getInstance().REG_LOGIN));%>?op=Google&contextURL=<%=request.getServletPath()%>" style="color: #333333; font-weight: bold; padding-right:5px;">LOG IN</a>
<%
        } else {
%>
